...
from model_permutation import permutation_spec_from_axes_to_perm
...
class Block(nn.Module):
    def __init__(self, in_chans, out_chans, stride, spatial_dim):
        ...
        self.conv1 = nn.Conv2d(
            in_chans, out_chans, kernel_size=3, stride=stride, padding=1, bias=False
        )
...
class ResNet(nn.Module):
    def __init__(self, wm=1, num_classes=10):
        super().__init__()
        ...
        for c, b, s, sd in zip(
            group_chans, group_blocks, group_strides, group_spatial_dims
        ):
            block_groups.append(
                BlockGroup(
                    in_chans=in_chans, out_chans=c, blocks=b, stride=s, spatial_dim=sd
                )
            )
            in_chans = c
    ...
    def _permutation_spec(self):
        """specifies specification for model permutation"""

        conv = lambda name, p_in, p_out: {f"{name}.weight": (p_out, p_in, None, None)}
        norm = lambda name, p: {f"{name}.weight": (p,), f"{name}.bias": (p,)}
        linear = lambda name, p_in, p_out: {
        ...
        # Block group 0
        perm_dict.update(
            {**block(f"block_groups.0.blocks.0", f"P_BG_0", f"P_BG_0_IN_0")}
        )
        perm_dict.update(
            {**block(f"block_groups.0.blocks.1", f"P_BG_0", f"P_BG_0_IN_1")}
        )
        perm_dict.update(
            {**block(f"block_groups.0.blocks.2", f"P_BG_0", f"P_BG_0_IN_2")}
        )
        ...
        perm_dict.update(
            {
                **shortcut_block(
                    f"block_groups.1.blocks.0", f"P_BG_0", f"P_BG_1_IN_0", "P_BG_1"
                )
            }
        )
        perm_dict.update(
            {**block(f"block_groups.1.blocks.1", f"P_BG_1", f"P_BG_1_IN_1")}
        )
        perm_dict.update(
            {**block(f"block_groups.1.blocks.2", f"P_BG_1", f"P_BG_1_IN_2")}
        )
        ...
        perm_dict.update(
            {
                **shortcut_block(
                    f"block_groups.2.blocks.0", f"P_BG_1", f"P_BG_2_IN_0", "P_BG_2"
                )
            }
        )
        perm_dict.update(
            {**block(f"block_groups.2.blocks.1", f"P_BG_2", f"P_BG_2_IN_1")}
        )
        perm_dict.update(
            {**block(f"block_groups.2.blocks.2", f"P_BG_2", f"P_BG_2_IN_2")}
        )
